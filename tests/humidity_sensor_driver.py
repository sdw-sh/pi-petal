...
HumidityMeasurement = namedtuple(
    "HumidityMeasurement", ["humidity", "requires_watering"]
)
...
class HumiditySensorDriver:
    ...
    def create_asd7830_based_controller(
        number_of_channels, default_watering_threshold=0.5
    ):
        ...
        if adc_device.detectI2C(0x4B):
            ...
            return HumiditySensorDriver(
                adc,
                number_of_channels,
                default_watering_threshold=0.5,
            )
        else:
    ...
    def check_humidity(self, channel):
        ...
        return HumidityMeasurement(
            humidity_value, humidity_value < self.humidity_thresholds[channel]
        )
